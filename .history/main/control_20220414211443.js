...
function renderNv(arr) {
  arr.forEach((nv, index) => {
    nv.employeeRating = employeeRating(nv);
    console.log(arr);
    const markup = `
    <tr id="nv-item">
        <td>${nv.username}</td>
        ...
        <td>${nv.employeeRating}</td>
        <td><button class="btn btn-danger btn-sm"  id="btn-delete">Delete</button></td>
    </tr>
    `;
    tableDanhSach.insertAdjacentHTML("beforeend", markup);
...
searchName.addEventListener("input", () => {
  const searchWord = searchName.value.toLowerCase();
  if (searchWord.trim().length === "") {
    tableDanhSach.innerHTML = "";
    renderNv(arrNv);
  } else {
    const newArrSearch = arrNv.filter((nv) =>
      nv.employeeRating.includes(searchWord)
    );
    tableDanhSach.innerHTML = "";
    renderNv(newArrSearch);
  }
});
